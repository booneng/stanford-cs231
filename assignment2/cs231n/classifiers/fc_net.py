...
def affine_relu_bn_forward(x, w, b, gamma, beta, bn_param):
    ...
    abn, bn_cache = batchnorm_forward(a, gamma, beta, bn_param)
    out, relu_cache = relu_forward(abn)
...
def affine_relu_bn_backward(dout, cache):
    ...
    dabn, dgamma, dbeta = batchnorm_backward_alt(da, bn_cache)
    dx, dw, db = affine_backward(dabn, fc_cache)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss_term = - np.sum(np.log(softmax_scores[range(N), y]))
        regularization_term = 0.5 * self.reg * (np.sum(self.params['W1']**2) + np.sum(self.params['W2']**2))
        loss = loss_term/N + regularization_term
        ...
        dA1, grads['W2'], grads['b2'] = affine_backward(dx, cache2)
        dX, grads['W1'], grads['b1'] = affine_relu_backward(dA1, cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        if self.use_dropout:
            dX = dropout_backward(dX, dropout_caches[-1])
        for i in range(self.num_layers - 1):
            cache = caches[-i-2]
            W = cache[1][1]
            if self.use_batchnorm:
