...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (gamma, x, x_norm, sample_mean, sample_var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    gamma, x, x_norm, u_b, sigma_squared_b, eps = cache
    N = x.shape[0]
    dx_norm = gamma * dout
    dx_1 = gamma * dout
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(hout):
        for j in range(wout):
            patch_x = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
                out[:, k, i, j] = np.sum(patch_x * w[k, :, :, :], axis=(1,2,3))
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(hout):
        for j in range(wout):
            patch_x = x_pad[:, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
            for k in range(F):
                dw[k ,: ,: ,:] += np.sum(patch_x * (dout[:, k, i, j])[:, None, None, None], axis=0)
            for n in range(N): #compute dx_pad
                dx_pad[n, :, i*stride:i*stride+HH, j*stride:j*stride+WW] += np.sum(w * dout[n, :, i, j].reshape(F, 1, 1, 1), axis=0)
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(hout):
        for j in range(wout):
            patch_x = x[:, :, i*stride:i*stride+ph, j*stride:j*stride+pw]
            out[:, :, i, j] = np.amax(patch_x, axis=(2, 3))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(hout):
        for j in range(wout):
            x_masked = x[:,:,i*stride : i*stride+ph, j*stride : j*stride+pw]
            max_x_masked = np.max(x_masked,axis=(2,3))
            temp_binary_mask = x_masked == max_x_masked[:, :, None, None]
            dx[:,:,i*stride : i*stride+ph, j*stride : j*stride+pw] += temp_binary_mask * dout[:,:,i,j][:,:,None,None]
